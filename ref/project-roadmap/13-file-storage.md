...
Files do **not** flow through our app server. The flow:

1. **Client → Core API:** "I want to upload a 2MB PDF named resume.pdf"
2. **Core API:** validates (size, mime, user permissions), generates a **presigned PUT URL** for S3, returns it
3. **Client → S3:** uploads directly via the presigned URL
4. **Client → Core API:** "Upload done, here's the file ID, attach it to my application"

This pattern:
- Saves our bandwidth (gigabytes per minute would crush our servers)
...
1. Client requests the file (e.g., `GET /attachments/{id}/download`)
2. Core API checks: is this user allowed to see this file?
