...
Each audience has its own UI app, its own login screen, and its own role model. The backend can be shared (modular monolith) or split (microservices) — but the **frontends are separate apps**.
